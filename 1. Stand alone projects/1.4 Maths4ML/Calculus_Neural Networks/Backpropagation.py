...
def reset_network (n1 = 6, n2 = 7, random=np.random) :
    ...
    W1 = random.randn(n1, 1) / 2 #Remember for the first iterations you set the parameters at random
...
def J_W2 (x, y) :
    ...
    J = J * d_sigma(z3)
    J = (J.T @ W3).T #Multiply by the Jacobian trans, since the Jacobian is a row vector. 
    # then the final lines are the same as in J_W3 but with the layer number bumped down.
    J = J * d_sigma(z2)
    J = J @ a1.T / x.size #Divided by N to compute the average of the sum of individual Jacobians
    return J
...
plot_training(x, y, iterations=15000, aggression=7, noise=1)
